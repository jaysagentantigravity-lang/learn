...
const VisualCard: React.FC<VisualCardProps> = ({ type, content, isPrompt = false }) => {
     ...
     if (imageUrl) {
        displayContent = (
           <img 
             ...
             className="w-full h-auto object-cover rounded-3xl shadow-2xl transition-all duration-500 hover:scale-[1.01] cursor-zoom-in" 
  ...
  return (
    <>
      ...
      >
        <div className="relative rounded-[40px] overflow-hidden border border-white/10 shadow-[0_0_40px_-10px_rgba(6,182,212,0.1)] bg-black/40 backdrop-blur-md group">
            ...
            {loading ? (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center overflow-hidden">
                ...
                <div className="relative grid grid-cols-3 gap-1 animate-pulse">
                   {[...Array(9)].map((_, i) => (
                      <div key={i} className="w-2 h-2 bg-cyan-500/40 rounded-full"></div>
                   ))}
                </div>
