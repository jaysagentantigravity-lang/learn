...
const MessageList: React.FC<MessageListProps> = ({ messages, onPlayAudio, onClarificationSubmit, isThinking, thinkingStatus }) => {
  ...
  const renderContent = (text: string) => {
    ...
    const regex = /!\[([\s\S]*?)\]\([\s\S]*?\)|\[DIAGRAM\]([\s\S]*?)\[\/DIAGRAM\]|\[\[GENERATE_IMAGE:.*?\]\]/g;
    const parts = [];
    let lastIndex = 0;
    let match;
    ...
    return parts.map((part, idx) => {
      ...
      return (
        <div key={idx} className="prose prose-invert max-w-none prose-lg">
          <ReactMarkdown
            components={{
              h1: ({node, ...props}) => <h1 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white via-zinc-300 to-zinc-500 mt-12 mb-8 tracking-tight leading-tight" {...props} />,
              h2: ({node, ...props}) => <h2 className="text-2xl md:text-3xl font-semibold text-zinc-100 mt-10 mb-6 border-l-4 border-zinc-500 pl-6 py-1" {...props} />,
              ...
              li: ({node, ...props}) => <li className="text-zinc-300 list-disc pl-2 marker:text-zinc-500" {...props} />,
              ...
              table: ({node, ...props}) => <div className="overflow-x-auto my-8 rounded-3xl border border-white/10 shadow-xl"><table className="min-w-full divide-y divide-white/10 bg-black/20" {...props} /></div>,
              thead: ({node, ...props}) => <thead className="bg-white/5" {...props} />,
              th: ({node, ...props}) => <th className="px-6 py-4 text-left text-xs font-bold text-zinc-400 uppercase tracking-wider" {...props} />,
                      ...
                      <>
                        <div className="relative z-10 min-h-[60px]">
                          ...
                          {isThinking && isLastMessage && (
                            <div className="h-6 w-2 bg-zinc-400 animate-pulse inline-block" />
                          )}
                        </div>
                          ...
                          <div className="mt-16 pt-8 border-t border-white/5 flex flex-wrap items-center justify-between gap-6">
                              ...
                              <AnimatePresence mode="popLayout">
                                {msg.groundingSources?.map((s, i) => (
                                  <motion.a 
                                    ...
                                    href={s.url} 
                                    target="_blank" 
                                    className="text-xs text-zinc-500 hover:text-white flex items-center gap-2 transition-colors px-3 py-1.5 rounded-full hover:bg-white/5 border border-transparent hover:border-white/10"
                                  >
                            ...
                            <div className="relative">
                              <button 
                                ...
                                className="flex items-center gap-3 px-6 py-3 rounded-full bg-white/5 hover:bg-white/10 text-zinc-300 hover:text-white transition-all border border-white/5 hover:border-white/20 group/btn"
                              >
                                <i className="fa-solid fa-volume-high text-lg group-hover/btn:text-white transition-colors"></i>
                              ...
                              <AnimatePresence>
                                {activeDropdownId === msg.id && (
                                  <motion.div
                                    ...
                                    className="absolute bottom-full right-0 mb-2 w-48 bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl overflow-hidden z-20"
                                  >
                                    <button
                                    ...
                                    >
                                      <i className="fa-solid fa-book-open text-zinc-400 text-sm"></i>
                                      Read Article
                                    </button>
                                    <button
                                    ...
                                    >
                                      <i className="fa-solid fa-microphone text-zinc-400 text-sm"></i>
                                      Explain it to me
                                    </button>
                                  </motion.div>
                                )}
