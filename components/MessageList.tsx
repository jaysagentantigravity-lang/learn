...
const CopyButton: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <button onClick={handleCopy} className="text-zinc-500 hover:text-white transition-colors flex items-center gap-1">
...
// Prevents recreating the component definition object on every render
...
const MessageItem = React.memo(({ msg, isLast, isThinking, onPlayAudio, onClarificationSubmit, onSuggestionClick }: MessageItemProps) => {
    ...
    const renderParts = (text: string) => {
        ...
        while ((match = regex.exec(text)) !== null) {
          ...
          } else if (match[3]) { // Standard ![alt](src)
        ...
        return parts.map((part, idx) => {
           ...
           return (
              <div key={idx} className="prose prose-invert max-w-none prose-lg">
                 <ReactMarkdown components={MARKDOWN_COMPONENTS}>{part.content}</ReactMarkdown>
              </div>
           );
        });
    };

    if (msg.role === 'model' && isLast && isThinking && !msg.text && !msg.storyManifest) {
        return null; // Curtain logic
    }
    ...
    return (
        <motion.div
            ...
            className="w-full"
            style={{ contentVisibility: isLast ? 'visible' : 'auto' }} // CSS Performance Boost
        >
            {msg.role === 'user' ? (
                    ...
                    {msg.text && (
                        <div className="mt-12 pt-8 border-t border-white/5 flex flex-wrap items-center justify-between gap-6">
                            ...
                            <div className="flex items-center gap-3 ml-auto">
                                <button 
                                    onClick={(e) => { e.stopPropagation(); onPlayAudio(msg, 'story'); }}
                                ...
                                <button 
                                    onClick={(e) => { e.stopPropagation(); onPlayAudio(msg, 'verbatim'); }}
    ...
    // Custom Equality for React.memo optimization
    // Only re-render if the message content actually changed or the thinking state changed for THIS specific message
    const sameText = prev.msg.text === next.msg.text;
    const sameState = prev.isThinking === next.isThinking;
    const sameLast = prev.isLast === next.isLast;
    
    // If it's a settled message (not last), we strictly check text stability
    if (!next.isLast) return true; // Never re-render old messages unless something drastic happens
    
    return sameText && sameState && sameLast;
});
